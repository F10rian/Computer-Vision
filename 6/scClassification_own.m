...
% setze Parameter
nBinsTheta = 3;            % Anzahl Bins für die Winkel von 0° bis 360°
nBinsR = 5;                % Anzahl Bins für Abstand von r_min bis r_max
rMin = 0.1;
rMax = 10;

% Shape Context der Trainingsdaten bestimmen
SC_train = zeros(nBinsR, nBinsTheta, size(train_data, 3), size(train_data, 3));
for it = 1:size(train_data, 3)
    ...
    %Y = scCompute([0.5, 0.5], X, nBinsTheta, nBinsR, rMin, rMax);
end
